...
namespace HirePathAI.API.Services.Implementations
{
    public class EvaluationService : IEvaluationService
    {
        ...
        public EvaluationService(IEvaluationRepository evalRepo, IApplicationRepository appRepo,
            IInterviewFeedbackRepository feedbackRepo, IUserRepository userRepo, ILogger<EvaluationService> logger)
        {
            _evalRepo = evalRepo;
            _appRepo = appRepo;
        ...
        private async Task<decimal> ComputeInterviewScoreAsync(int jobApplicationId)
        {
            ...
            if (feedback.Count == 0) return 0m;

            // Feedback scores use a 1-10 scale. Convert the average to 0-100
            // before combining it with resume and AI scores.
            var averageOutOfTen = feedback.Average(f =>
                (f.TechnicalScore + f.CommunicationScore + f.ProblemSolvingScore) / 3.0m);
            return Math.Round(averageOutOfTen * 10m, 2);
        }

        public async Task<Evaluation> CreateOrUpdateAsync(int jobApplicationId, decimal? resumeScore,
            decimal? aiScore, int actingUserId, bool isAdmin)
        {
            ...
            var overallScore = Math.Round(finalResumeScore * ResumeWeight + finalAiScore * AIWeight +
                                          interviewScore * InterviewWeight, 2);
            ...
            _logger.LogInformation("Evaluation for application {ApplicationId} calculated as {OverallScore} by user {UserId}",
                jobApplicationId, overallScore, actingUserId);
            return evaluation;
        }
        ...
        public async Task<Evaluation?> GetByApplicationIdAsync(int jobApplicationId, int actingUserId, bool isAdmin)
        {
            ...
            if (!await HasCompanyAccessAsync(application.Job, actingUserId, isAdmin)) return null;
